@props([
    'type' => 'info',
    'title' => null,
    'message' => null,
    'dismissible' => false,
    'showIcon' => true,
    'class' => '',
    'id' => null
])

@php
    // Base classes
    $baseClasses = 'rounded-md p-4';
    
    // Type-specific classes
    $typeClasses = match($type) {
        'success' => 'bg-green-50 border border-green-200',
        'error' => 'bg-red-50 border border-red-200',
        'warning' => 'bg-yellow-50 border border-yellow-200',
        'info' => 'bg-blue-50 border border-blue-200',
        default => 'bg-blue-50 border border-blue-200'
    };
    
    // Text color classes
    $textClasses = match($type) {
        'success' => 'text-green-800',
        'error' => 'text-red-800',
        'warning' => 'text-yellow-800',
        'info' => 'text-blue-800',
        default => 'text-blue-800'
    };
    
    // Icon color classes
    $iconClasses = match($type) {
        'success' => 'text-green-400',
        'error' => 'text-red-400',
        'warning' => 'text-yellow-400',
        'info' => 'text-blue-400',
        default => 'text-blue-400'
    };
    
    // Button color classes for dismissible alerts
    $buttonClasses = match($type) {
        'success' => 'text-green-500 hover:text-green-600 focus:ring-green-600',
        'error' => 'text-red-500 hover:text-red-600 focus:ring-red-600',
        'warning' => 'text-yellow-500 hover:text-yellow-600 focus:ring-yellow-600',
        'info' => 'text-blue-500 hover:text-blue-600 focus:ring-blue-600',
        default => 'text-blue-500 hover:text-blue-600 focus:ring-blue-600'
    };
    
    // Combine classes
    $finalClasses = trim($baseClasses . ' ' . $typeClasses . ' ' . $class);
    
    // Generate ID if not provided
    $alertId = $id ?? 'alert-' . $type;
@endphp

<div 
    id="{{ $alertId }}" 
    class="{{ $finalClasses }}"
    role="alert"
    aria-live="polite"
    {{ $attributes }}
    @if($dismissible) x-data="{ show: true }" x-show="show" x-transition @endif
>
    <div class="flex">
        @if($showIcon)
            <div class="flex-shrink-0">
                @switch($type)
                    @case('success')
                        <svg class="h-5 w-5 {{ $iconClasses }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                        </svg>
                        @break
                    @case('error')
                        <svg class="h-5 w-5 {{ $iconClasses }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                        </svg>
                        @break
                    @case('warning')
                        <svg class="h-5 w-5 {{ $iconClasses }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                        </svg>
                        @break
                    @case('info')
                    @default
                        <svg class="h-5 w-5 {{ $iconClasses }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                        </svg>
                        @break
                @endswitch
            </div>
        @endif
        
        <div class="@if($showIcon) ml-3 @endif flex-1">
            @if($title)
                <h3 class="text-sm font-medium {{ $textClasses }}">
                    {{ $title }}
                </h3>
            @endif
            
            @if($message || $slot->isNotEmpty())
                <div class="@if($title) mt-2 @endif text-sm {{ $textClasses }}">
                    @if($message)
                        <p>{{ $message }}</p>
                    @endif
                    
                    @if($slot->isNotEmpty())
                        {{ $slot }}
                    @endif
                </div>
            @endif
        </div>
        
        @if($dismissible)
            <div class="ml-auto pl-3">
                <div class="-mx-1.5 -my-1.5">
                    <button 
                        type="button" 
                        class="inline-flex rounded-md p-1.5 {{ $buttonClasses }} hover:bg-opacity-20 focus:outline-none focus:ring-2 focus:ring-offset-2"
                        @click="show = false"
                        aria-label="Cerrar alerta"
                    >
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>
                </div>
            </div>
        @endif
    </div>
</div>